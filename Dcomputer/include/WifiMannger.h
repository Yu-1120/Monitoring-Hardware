...
const int resetPin = 12;                    //设置重置按键引脚,用于删除WiFi信息  ESP32-C3是12口是复位
const int LED = 2;                         //设置LED引脚
const char* AP_SSID  = "ESP32C3-Monitor-hardware";    //设置AP热点名称
...
String wifi_ssid = "";                     //暂时存储wifi账号密码II
...
WebServer WebserverMannger(webPort);      //开启web服务, 创建TCP SERVER,参数: 端口号,最大连接数
void handleRoot();
void handleNotFound();
void initSoftAP();
void initDNS();
...
void handleRoot(){
  if (WebserverMannger.hasArg("selectSSID")){
      WebserverMannger.send(200, "text/html", ROOT_HTML + scanNetworksID + "</body></html>");
    }else{
      WebserverMannger.send(200, "text/html", ROOT_HTML + scanNetworksID + "</body></html>");
    }     
}
...
void initWebServer(){
 ...
 //必须添加第二个参数HTTP_GET，以下面这种格式去写，否则无法强制门户
  WebserverMannger.on("/", HTTP_GET, handleRoot);                      //  当浏览器请求服务器根目录(网站首页)时调用自定义函数handleRoot处理，设置主页回调函数，必须添加第二个参数HTTP_GET，否则无法强制门户
  WebserverMannger.on("/configwifi", HTTP_POST, handleConfigWifi);     //  当浏览器请求服务器/configwifi(表单字段)目录时调用自定义函数handleConfigWifi处理
  WebserverMannger.onNotFound(handleNotFound);                         //当浏览器请求的网络资源无法在服务器找到时调用自定义函数handleNotFound处理 
  //Tells the server to begin listening for incoming connections.Returns None
  WebserverMannger.begin();                                           //启动TCP SERVER
...
void connectToWiFi(int timeOut_s){
      Serial.println("Enter the connectToWiFi() function");//进入connectToWiFi()函数
      //设置为STA模式并连接WIFI
      WiFi.mode(WIFI_STA);
      WiFi.setAutoConnect(true);//设置自动连接
      //用字符串成员函数c_str()生成一个const char*指针，指向以空字符终止的数组,即获取该字符串的指针。
      if(wifi_ssid !=""){
          Serial.println("Connect using web configuration information.");  //用web配置信息连接
          
          WiFi.begin(wifi_ssid.c_str(), wifi_pass.c_str());
          wifi_ssid = "";
          wifi_pass = "";
        }else{
           Serial.println("Connect information saved with NVS.");//用nvs保存的信息连接
...
void handleConfigWifi(){
      ...
      if (WebserverMannger.hasArg("ssid")) {//判断是否有账号参数
        Serial.print("got ssid:");
        wifi_ssid = WebserverMannger.arg("ssid");      //获取html表单输入框name名为"ssid"的内容
        // strcpy(sta_ssid, server.arg("ssid").c_str());//将账号参数拷贝到sta_ssid中
        Serial.println(wifi_ssid);
      }else{//没有参数
        Serial.println("error, not found ssid");
        WebserverMannger.send(200, "text/html", "<meta charset='UTF-8'>error, not found ssid");//返回错误页面
        return;
      }
      //密码与账号同理
      if (WebserverMannger.hasArg("pass")) {
        Serial.print("got password:");
        wifi_pass = WebserverMannger.arg("pass");       //获取html表单输入框name名为"pwd"的内容
        //strcpy(sta_pass, server.arg("pass").c_str());
        Serial.println(wifi_pass);
      }else{
        Serial.println("error, not found password");
        WebserverMannger.send(200, "text/html", "<meta charset='UTF-8'>error, not found password");
        return;
      }
      WebserverMannger.send(200, "text/html", "<meta charset='UTF-8'>SSID："+wifi_ssid+"<br />password:"+wifi_pass+"<br />已取得WiFi信息,正在尝试连接,请手动关闭此页面。");//返回保存成功页面      
      delay(2000);    
      WiFi.softAPdisconnect(true);     //参数设置为true，设备将直接关闭接入点模式，即关闭设备所建立的WiFi网络。 
      WebserverMannger.close();                  //关闭web服务       
      WiFi.softAPdisconnect();         //在不输入参数的情况下调用该函数,将关闭接入点模式,并将当前配置的AP热点网络名和密码设置为空值.    
      Serial.println("WiFi Connect SSID:" + wifi_ssid + "  PASS:" + wifi_pass);   
      if(WiFi.status() != WL_CONNECTED){
        Serial.println("Start calling the connection function connectToWiFi()");//开始调用连接函数connectToWiFi()..
        connectToWiFi(connectTimeOut_s);
      }else{
        Serial.println("The submitted configuration information was automatically connected successfully..");//提交的配置信息自动连接成功..
      }    
}
...
void restoreWiFi(){
       ...
       Serial.println("Connection information cleared, preparing to restart device");//连接信息已清空,准备重启设备..
       delay(10);           
    ...
    if(WiFi.status() != WL_CONNECTED){
      ...
      //  Serial.println(WiFi.status()); 
        ...
        if(reConnect == true && WiFi.getMode() != WIFI_AP && WiFi.getMode() != WIFI_AP_STA ){
            ...
            Serial.println("Connecting to WiFi...");//正在连接WiFi...
            connectToWiFi(connectTimeOut_s);
        }  
...
void   ExitWifiManngerInit(){

    //长按5秒(P0)清除网络配置信息
    if(!digitalRead(resetPin)){
        delay(5000);
        if(!digitalRead(resetPin)){   
           Serial.println("\nPress and hold the button for 5 seconds, clearing network connection and saving connection information");  //按键已长按5秒,正在清空网络连保存接信息.
           restoreWiFi();    //删除保存的wifi信息 
           ESP.restart();    //重启复位esp32
           Serial.println("Device restarted.");//已重启设备
        }      
     }
 
    dnsServer.processNextRequest();   //检查客户端DNS请求
    WebserverMannger.handleClient();            //检查客户端(浏览器)http请求
