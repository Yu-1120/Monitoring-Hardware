...
const char* ssid     = "ikun";       // 连接WiFi名（此处使用shuo为示例）
                                            // 请将您需要连接的WiFi名填入引号中
const char* password = "hongpeiyu";          // 连接WiFi密码（此处使用123456789为示例）
                                            // 请将您需要连接的WiFi密码填入引号中
//===========================================
//初始化
TFT_eSPI tft;
...
void parseInfo(WiFiClient client){
  ...
  //
  tft.println("City城市"+results_location_name);     //打印文字（字体有设置才有用）  这个天气返回的时候会中文乱码  
  Serial.println(F("========================"));
  tft.println(results_0_now_temperature_str);//Tem:xxx
  //体感温度:
...
void loop() {
  ...
  // tft.fillScreen(TFT_BLACK);  //刷屏底色为黑色
  // tft.setCursor(0, 10);       //设置光标位置
  // tft.setTextFont(6);  //支持2，4号大小,5号大小不支持了
